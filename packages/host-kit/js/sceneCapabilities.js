...
function authorizeSceneHostTslCode({ hash, source, code, notice } = {}) {
  ...
  return globalThis.confirm([
    "此场景请求运行 TSL JavaScript 模块。批准后，该模块拥有与 SceneHost 页面相同的权限。",
    ...
    "",
    "Run this TSL JavaScript module? It receives the same page permissions as SceneHost."
  ].filter(Boolean).join("\n"));
}

/** SceneHost is an advanced authoring host, so it activates the optional WebGPU entry after
 * negotiation selects it (or before replaying a descriptor that already requires it). This
 * changes only SceneHost's host inventory; ordinary WebGL scenes never load the entry. */
export async function activateSceneHostAiCapabilities({ tslCode = false } = {}) {
  if (tslCode) {
      ...
      tslCodeActivationPromise = import("threejson/tsl-code")
        .then((module) => {
          module.configureTslCodeExecution({
            ...
            authorize: authorizeSceneHostTslCode
          });
...
export async function ensureSceneHostSceneCapabilitiesForPayload(payload) {
  if (!scenePayloadRequiresWebgpu(payload)) return false;
  const needsTslCode = scenePayloadRequiresTslCode(payload);
  const activated = await activateSceneHostAiCapabilities({ tslCode: needsTslCode });
  if (!activated) {
    const activationError = needsTslCode ? tslCodeActivationError : webgpuActivationError;
    const error = new Error(
      `SceneHost could not activate the WebGPU/TSL runtime: ${activationError?.message || activationError || "unknown error"}`
    );
    error.code = "E_SCENE_HOST_WEBGPU_ACTIVATION_FAILED";
...
export function shouldActivateSceneHostTslCode({
  scene,
...
export function resolveSceneHostAiRendererBackend({
  scene,
  selectedCapabilityIds,
  matchedCapabilityIds
} = {}) {
