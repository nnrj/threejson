...
export function createThreeBoxBuiltinNotifications(settingsProvider) {
  ...
  function render() {
    ...
    bell.innerHTML = `${BELL_ICON}${unread ? `<em aria-label="${unread} 条未读通知">${unread > 99 ? "99+" : unread}</em>` : ""}`;
    ...
    for (const notification of list) {
      ...
      row.innerHTML = `<b>${escapeHtml(notification.title)}</b><div class="threeboxNoticeBody">${notificationBodyHtml(notification)}</div><small>${new Date(notification.created_at || Date.now()).toLocaleString()}</small>`;
      secureLinks(row);
  ...
  function toast(notification, base) {
    ...
    node.innerHTML = `<strong>${escapeHtml(notification.title)}</strong><div class="threeboxBuiltinNoticeBody">${notificationBodyHtml(notification)}</div>`;
    secureLinks(node);
    ...
    toastNode = node;
    try { localStorage.setItem(lastPopupStorageKey(base), notification.id); } catch { /* optional */ }
  ...
  async function poll() {
    ...
    try {
      ...
      ensureUi();
      render();
      const newestPopup = selectNewestPopupNotification(list, hasPolled ? seen : new Set());
      for (const notification of list) seen.add(notification.id);
      if (!hasPolled) {
        let lastPopupId = "";
        try { lastPopupId = localStorage.getItem(lastPopupStorageKey(current.base)) || ""; } catch { /* optional */ }
        if (newestPopup && newestPopup.id !== lastPopupId) toast(newestPopup, current.base);
      } else if (newestPopup) toast(newestPopup, current.base);
      hasPolled = true;
    } catch {
      // Notifications must never interrupt the scene workflow.
    }
  }
