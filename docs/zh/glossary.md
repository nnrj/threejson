...
| 正本 | canonical pages | 根目录 `scene-editor.html` / `scene-player.html`；当前稳定推荐入口 | Root `scene-editor.html` / `scene-player.html`; current stable entry | [`docs/tools.md`](./tools.md#scene-host-绿场拆分重构--对齐中) |
| 绿场 | greenfield (`scene-host`) | `tools/scene-host/` 对正本的模块化拆分重构，对齐调试中 | Modular refactor of canonical pages under `tools/scene-host/` | [`docs/tools.md`](./tools.md) |
