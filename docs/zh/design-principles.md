...
- **行为可预测**：没有相应 JSON 描述符时，不加载、不初始化、不发起网络请求。Manifest 中明确标为 `activation: "descriptor"` 的稳定、轻量、随包模块，可由异步场景加载器在描述符实际引用时按需加载；重依赖、第三方后端和代码执行能力仍必须由宿主显式 import/注册，是否要求额外授权由宿主策略决定。Manifest 声明、加载行为、文档和 AI 能力目录必须一致。

## 能力与策略分离

ThreeJSON 是场景引擎。只要不存在客观技术障碍，Three.js 能稳定暴露、又能被合理描述或注册的通用能力，就应由引擎提供，而不应因为某一个上层应用面对不可信内容，就在引擎层统一删减效果、禁用模块依赖或降低表达上限。

- **引擎负责能力**：定义描述符、装配运行时、提供扩展点、诊断、资源释放和必要的策略钩子。
- **宿主负责策略**：根据内容来源和部署环境，选择可信直通、逐次确认、受限执行或完全禁用；社区站点、编辑器、离线工具和私有应用可以采用不同策略。
- **安全模块负责可选约束**：沙箱、来源白名单、内容签名、CSP、审核和组织策略应作为宿主或额外安全模块叠加，而不是冒充渲染能力本身。

“能力完整”不等于“默认加载一切”。WebGPU、TSL 代码等能力仍通过显式子路径启用，以保持默认零开销；但一旦宿主选择启用，引擎应提供完整可用的机制，并把最终信任决策留给宿主。只有确实无法可靠实现的组合才应报错，同时给出结构化原因，而不是静默降级或全局封禁。

资源预算同样属于策略：core 可以接收宿主显式传入的顶点、字节、时间等预算并返回结构化诊断，但默认值应为不限制。不得为了估算 Token、费用或设备性能，在 core 中写死顶点数、面数、场景字节数或 AI 续写轮数，也不得在未获用户授权时把自由形态模型偷偷替换成基础几何。
