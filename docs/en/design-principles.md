...
“JSON is king” refers to the **authoring document**, not each rendered frame. Document sessions own immutable parameters, material sources, topology and history. `userData.objJson` and `worldInfo` are compiled/runtime descriptor views and must not overwrite that source. Low-level imperative APIs retain their descriptor-sync contract. Game loops, physics, Mixer and scripts may modify `THREE.Object3D` directly; authoring and playback need not match every frame. See the [authoring/runtime migration guide](../dev/scene-authoring-runtime.md).

- **Game / high-frequency paths**: Runtime-only updates remain valid. For explicit pose capture, imperative hosts can use `reconcileTransformToDescriptor`; session hosts commit the selected changes as document transactions. Routine save must not capture animation frames, helper objects or transient resource state. See [scope.md](./scope.md).
...
6. **Editor vs snapshot**: authoring persistence uses document sessions; `sceneToJson` remains an explicit runtime capture API. Playback, resources and viewports have separate lifetimes. Core supplies generic transactions without knowing a specific editor host.
