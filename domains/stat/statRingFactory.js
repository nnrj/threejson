/**
 * stat.ring: cylinder sector slices with CSG inner holes (requires three-bvh-csg in import map).
 */
 ...
 * @returns {object}
 */
export function buildStatRingGroupJson(record = {}, options = {}) {
  const mergedOptions = {
    ...options,
    ...(record.options && typeof record.options === "object" ? record.options : {})
  };
  const slices = record.slices ?? mergedOptions.slices ?? [];
  const outerRadius = Number(mergedOptions.outerRadius ?? mergedOptions.radius) || 40;
  const innerRadius = Number(mergedOptions.innerRadius) || 22;
  const height = Number(mergedOptions.height) || 14;
  const radialSegments = Number(mergedOptions.radialSegments) || 48;
  ...
  const labelStyle =
    mergedOptions.labelStyle && typeof mergedOptions.labelStyle === "object"
      ...
      : undefined;
  const statKind = mergedOptions.statKind || "stat.ring";
  const groupName = record.name || mergedOptions.name || "statRing";
  const labelMidRadius = (outerRadius + innerRadius) / 2;
  const holeHeight = height + 0.04;
  const ranges = sliceToThetaRanges(slices, {
    startAngle,
    maxSlices: STAT_RING_MAX_SLICES
  });
  /** @type {object[]} */
  const subScene = [];
  ...
  return {
    name: groupName,
    objType: mergedOptions.groupObjType || "statRingGroup",
