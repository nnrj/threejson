...
> **非发布承诺**（`idea`）。记录当前 JSON schema 与加载实现之间的差异，供后续评估是否将 runtime 项统一为 `objType` + registry。划界原则见 [`doc/design-principles.md`](../doc/design-principles.md) §Core 与 extensions 的划界。
...
- [`doc/design-principles.md`](../doc/design-principles.md) — Core 与 extensions 划界
