...
**状态**：`idea`（容器形状备忘；正式接入见 [`doc/extensions.md`](../doc/extensions.md)）

正式接入说明见 **[doc/extensions.md](../doc/extensions.md)**。本文保留容器形状与字段合并规则摘要。
...
编辑器可选：`sceneConfig.pick.meshBvh` 启用 BVH 拾取（见 [`doc/tools.md`](../doc/tools.md) 中 settings 与 JSON 合并说明）。
