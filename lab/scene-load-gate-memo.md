...
**状态**：`idea`（开放探索，**不承诺实现**）  
**关联**：[`sceneConfig.intro`](../doc/json-format.md#sceneconfigintro-可选加载完成后片头)、[`deployScheduler`](../core/runtime/deployScheduler.js)、[`scene-load-lifecycle.md`](../doc/scene-load-lifecycle.md)  
**记录日期**：2026-07-05  

本页记录 load gate（「何时算加载完成」）的扩展设想。与 [`doc/scope.md`](../doc/scope.md) 的 Core 承诺区分。
